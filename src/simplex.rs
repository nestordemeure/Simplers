...
impl<CoordFloat: Float, ValueFloat: Float> Simplex<CoordFloat, ValueFloat> {
    ...
    fn new(
        corners: Vec<Rc<Point<CoordFloat, ValueFloat>>>,
        ratio: ValueFloat,
        ...
        F: FnMut(&[CoordFloat]) -> ValueFloat,
    {
        ...
        let mut corners: Vec<_> = (0..search_space.dimension)
            .map(|i| {
    ...
    pub fn split(self, new_point: Rc<Point<CoordFloat, ValueFloat>>, difference: ValueFloat) -> Vec<Self> {
        ...
        let distances: Box<[ValueFloat]> = self
            .corners
            .iter()
            .map(|c| &c.coordinates)
    ...
    pub fn evaluate(&self, exploration_depth: ValueFloat) -> ValueFloat {
        ...
        let inverse_distances: Vec<ValueFloat> = self
            .corners
            .iter()
